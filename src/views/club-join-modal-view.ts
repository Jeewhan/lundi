...
export class ClubJoinModalView {
    ...
    return {
      ...
      view: {
        ...
        blocks: [
          ...
          ...(false
            ? []
            : [{ type: "divider" }, ...this.getPolicyAgreementBlocks()]),
        ],
      },
    };
  }
  ...
  private getNameVerificationBlocks() {
    return [
      ...(this.user
        ? [
            {
  ...
  private getPolicyAgreementBlocks(): KnownBlock[] {
    return [
...

      {
        ...
        label: {
          type: "plain_text",
          text: "일정이 확정되면 꼭 참여해주세요!\n혹시나 일정변경이 필요하다면 바로 공유해주세요.",
          emoji: true,
        },
      },
...

      {
        ...
        label: {
          type: "plain_text",
          text: "클럽 운영을 위한 개인정보 수집·이용 및 제3자 제공 동의에 관한 항목입니다.\n개인정보 제3자 제공에 동의하실까요?",
          emoji: true,
        },
      },
    ];
  ...
  private getDinnerClubTopicSelectionBlocks(): KnownBlock[] {
    return [
...

      {
        ...
        label: {
          type: "plain_text",
          text: "오프라인 모임을 위해 선호하는 지역을 골라주세요.(복수선택 가능)\n- 많이 고를수록 더 다양한 만남이 가능해요.",
          emoji: true,
        },
      },
    ];
